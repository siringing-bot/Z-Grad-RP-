...
function ENT:Draw()
    ...
    cam.Start3D2D(pos, Angle(0, ang.y, 90), 0.1)
        draw.SimpleTextOutlined("Контрабандист", "DermaLarge", 0, 0, Color(60, 60, 60), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0))
        draw.SimpleTextOutlined("Нажмите E", "DermaDefault", 0, 30, Color(200, 200, 200), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0))
    cam.End3D2D()
end

net.Receive("ZGrad_Contraband_OpenShop", function()
    ...
    frame.Paint = function(self, w, h)
        ...
        draw.SimpleText("🏴‍☠️ Контрабандист — Магазин", "DermaDefaultBold", w / 2, 15, Color(220, 200, 150), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    ...
    local isCrim = ZGrad_Utility.IsCriminal(LocalPlayer())

    for _, catData in ipairs(categories) do
        local items = ShopItems[catData.id]
        if items and #items > 0 then
            -- Check if we have visible items in this category
            local hasVisible = false
            for _, item in ipairs(items) do
                if not item.isCriminal or isCrim then
                    hasVisible = true
                    break
                end
            end
            ...
            if hasVisible then
                ...
                for i, item in ipairs(items) do
                    if item.isCriminal and not isCrim then continue end
                    ...
                    btnBuy.DoClick = function()
                        net.Start("ZGrad_Contraband_BuyItem")
